...
// 以下兩個函式的內容是從 AmberApplication.cpp 的 main() **逐字搬過來的**，
// 只做了兩件事：
//   1. 開頭加一段 const 參考別名，讓搬過來的程式碼不必改寫變數名；
//   2. 原本 main() 的 `return 0;` 早退改成 PrescanResult.hasBamStage。
// 除此之外沒有任何邏輯變更。行為出處的註解全部原樣保留。

PrescanResult prescan(const PipelineConfig &cfg)
{
    const std::string &lociPath = cfg.lociPath;
    const std::string &bedPath = cfg.bedPath;
    const std::string &tumorBam = cfg.tumorBam;
    const std::string &debugOnlyChr = cfg.debugOnlyChr;
    ...
    result.hasBamStage = true;

    std::vector<amber::PositionEvidence> &evidence = result.evidence;
    std::vector<amber::RegionTask> &tasksOut = result.tasks;
        ...
        const BamScanStats &stats)
{
    ...
    const std::string &outputDir = cfg.outputDir;
    const std::string &sampleId = cfg.sampleId;
    const bool writeTumorData = cfg.writeTumorData;
    const bool writeVersion = cfg.writeVersion;
