...
void unknown(std::vector<Combined>&v,double normalCopyNumber){for(int i=0;i<(int)v.size();++i)if(!v[i].processed()){v[i].method=CopyNumberMethod::UNKNOWN;v[i].value.tumorCopyNumber=normalCopyNumber;if(v[i].value.segment.support==SegmentSupport::NONE&&i>0&&v[i-1].method==CopyNumberMethod::UNKNOWN){v[i-1].extend(v[i].value);v.erase(v.begin()+i);--i;}}}
...
std::vector<PurpleCopyNumber> buildCopyNumbers(const std::vector<ObservedRegion>&regions,const FittedPurity&fit,Gender gender){std::vector<PurpleCopyNumber>out;for(int chr=1;chr<=24;++chr){std::string key=chr==23?"X":chr==24?"Y":std::to_string(chr);std::vector<ObservedRegion>cv;for(auto&r:regions)if(lp::stripChrPrefix(r.segment.chromosome)==key)cv.push_back(r);auto c=extendDiploid(cv,fit.purity);longArm(c);double normalCn=(gender==Gender::MALE&&(key=="X"||key=="Y"))?1:2;unknown(c,normalCn);inferBafs(c);auto x=convert(c);out.insert(out.end(),x.begin(),x.end());}return out;}
